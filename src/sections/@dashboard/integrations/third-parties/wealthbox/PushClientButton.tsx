...
// sections
import ConflictClientsDialog from './ConflictClientsDialog';
// redux
...
export default function WealthboxPushClientButton({
  client,
  integrationOwner,
  ...
  const [conflictClients, setConflictClients] = useState<WealthboxContact[]>([]);
  const [contactId, setContactId] = useState<WealthboxContact['id'] | null | undefined>();
  ...
  const fetchClientList = async () => {
    try {
      ...
      const response = await findAllClientFromIntegrationByEmail({
        integrationType: serviceIds.WEALTHBOX as IntegrationServiceId.WEALTHBOX,
        email: client.email,
        integrationOwner,
  ...
  const handlePush = async (contactId?: string) => {
    try {
      ...
      handleCloseConfirm();
      enqueueSnackbar('Client pushed to Wealthbox!', { variant: 'success' });
      setIsLoading(false);
      return;
    } catch (error: any) {
      console.error(error);
      if (error?.response?.data.code === 409) {
        // Multiple contacts were found, show list
        setConflictClients(error.response?.data.contacts);
      } else {
        let errorMessage;
        if (typeof error.response?.data.error === 'string') {
          errorMessage = error.response?.data.error;
        } else if (error.response?.data.error?.errors?.length) {
  ...
  return (
    ...
    >
      ...
      >
        Push Client to Wealthbox{showIntegrationOwner && ` (${sentenceCase(integrationOwner)})`}
      </LoadingButton>

      {clientHasUpdates && (
        <Tooltip title="The client has new changes since they were last pushed to Wealthbox">
      ...
      {lastPushedAt && (
        <Typography
          textAlign="center"
          variant="caption"
          sx={{ fontStyle: 'italic', mx: 'auto', textAlign: 'center' }}
        >
      ...
      <Dialog open={confirmOpen} maxWidth="xs">
        ...
        <DialogContent>
          <DialogContentText>
            This will update this client in Wealthbox, or create a new contact in Wealthbox if one
            does not exist yet.{' '}
            <strong>
      ...
      <ConflictClientsDialog
        type="push"
        conflictClients={conflictClients}
        selectedContactId={contactId}
        handleCancel={handleCloseConfirm}
        handleSelect={(id) => setContactId(id)}
        handleContinue={handlePush}
        isLoading={isLoading}
      />
      <SelectClientDialog
        onSelect={handlePush}
