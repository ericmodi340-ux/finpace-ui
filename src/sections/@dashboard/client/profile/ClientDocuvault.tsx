...
const TABLE_HEAD = [
  { id: 'key', label: 'File Name' },
  { id: 'size', label: 'Size' },
  { id: 'lastModified', label: 'Last Modified' },
  { id: 'cta', label: '' },
];

const ClientDocuvault = ({ clientId }: Props) => {
  ...
  const getDocuvaultList = useCallback(
    async (clientId: string) => {
      try {
        setLoading(true);
        const response = await Storage.list(`clients/${clientId}/`, {
          pageSize: 'ALL',
        });
  ...
  useEffect(() => {
    getDocuvaultList(clientId);
  }, [clientId, getDocuvaultList]);
      ...
      if (clientId) {
        ...
        const uploadPromises = files.map(async (file: any) => {
          await createStorageItem({
            path: `clients/${clientId}/docuvault/${file.name}`,
            file: file,
        ...
        await getDocuvaultList(clientId);
        setFiles([]);
        enqueueSnackbar(
  ...
  return (
    <div>
      <Card sx={{ py: 3 }}>
        <Typography mb={3} mx={2} variant="h5">
          DocuVault
        </Typography>
        ...
        {loading ? (
          ...
          <CustomTable
            ...
            tableData={docuvaultList}
            elementWrappers={{
              key: (row: any) => <Typography>{row.key.split('/').pop()}</Typography>,
              size: (row: any) => <Typography>{fData(row.size)}</Typography>,
              lastModified: (row: any) => <Typography>{fDateTime(row.lastModified)}</Typography>,

              cta: (row: any) => (
                <Stack direction="row" spacing={1}>
                  <IconButton onClick={() => handleDownload(row.key)}>
                    <Iconify icon="material-symbols:download" />
                  </IconButton>
                  <DeleteDocuvaultFile row={row} setDocuvaultList={setDocuvaultList} />
                </Stack>
              ),
            }}
          />
        ...
        {docuvaultList.length === 0 && !loading && (
          <Typography variant="body1" sx={{ color: 'text.secondary', textAlign: 'center', my: 5 }}>
            No documents found!
          </Typography>
        )}
      </Card>
      <Card sx={{ py: 3, px: 3, textAlign: 'left', mt: 3 }}>
...
export default ClientDocuvault;
